...
def rename(t):
    r"""Replace every variant, treating a literal \n as a word boundary.

    Captions store a line break as the two characters BACKSLASH and 'n', not as
    0x0A. So in "Don't mess with me,\nRaben!" the character before the R is the
    LETTER n, and \bRaben\b finds no boundary and never matches. Two captions
    survived the first pass for exactly this reason.

    Swapping the literal \n for a non-word sentinel before the regex, and back
    afterwards, makes the boundary real without special-casing each variant."""
    x = t.replace(b"\\n", SENTINEL)
    for w in VARIANTS:
        x = re.sub(br"\b%s\b" % w, GOOD, x)
    return x.replace(SENTINEL, b"\\n")

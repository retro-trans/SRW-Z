# -*- coding: utf-8 -*-
"""Two mechanical text bugs in the DeepSeek records, both visible in-game.

1. 336 rows contain a LITERAL backslash-n instead of a line break, so the
   player reads "Sorry...\nAll I can do is apologize...". DeepSeek emitted the
   escape as text and nothing downstream turned it into a newline.
2. 195 rows use the fullwidth "．" inside dialogue. Fullwidth punctuation is
   correct for the MENU reader (the encyclopedia), where ASCII 0x2E is a
   control code - but dialogue uses ASCII, so these render inconsistently.

Both were found by proofreading agents; fixing them by script keeps the quota
for the errors that actually need Japanese to resolve.

Rows are located by re-resolving the pointer at apply time; rows that no longer
fit are re-wrapped, and rows that outgrow their slot are relocated.

Usage: fix_textbugs.py <iso> [--dry-run]
"""
...
def fix_text(en):
    return en.replace(chr(92) + "n", chr(10)).replace(u"．", ".")

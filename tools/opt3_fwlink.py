# -*- coding: utf-8 -*-
"""Fullwidth-link test in the corridor (scene) box + in-game, vs halfwidth."""
...
OUT_ISO  = os.path.join(WORK, "iso", "srwz_fwlink.bin")
...
GSf = fw("Glory Star")
GSh = "Glory Star"
def link(term): return OB+LO+term+LC+CB
TESTS = {
    # in-game
    7:  ("Denzel", ["\u300cFull: "+link(GSf)+"\u300d"]),
    8:  ("Toby",   ["\u300cHalf: "+link(GSh)+"\u300d"]),
    9:  ("Denzel", ["\u300cFull mid: the "+link(GSf),
                    "\u3000team, tidy now?\u300d"]),
    # scene / corridor
    142:("Jerid",  ["\u300cFull: "+link(GSf)+"\u300d"]),
    144:("Setsuko",["\u300cHalf: "+link(GSh)+"\u300d"]),
    146:("Setsuko",["\u300cFull mid: I am from the",
                    "\u3000"+link(GSf)+" squad now.\u300d"]),
}
def build_string(r):
